"""
PATCHRIGHT RUNNER - Browser-Level Stealth Patches

Patchright is a patched version of Playwright with:
- Chrome binary modifications
- Automated removal of automation markers
- Better fingerprint consistency
- Maintained stealth patches

Best for: Good stealth with minimal configuration
"""
...
class PatchrightRunner(BaseRunner):
    """Patchright runner with browser-level stealth"""
    
    def __init__(self, screenshot_engine=None):
        super().__init__(screenshot_engine)
        logger.info("Patchright runner initialized (browser-level stealth)")
    ...
    async def run_test(
        ...
        """Run test with Patchright's browser-level patches"""
        ...
        try:
            async with async_playwright() as p:
                ...
                # Patchright has built-in patches, simple launch
    ...
    async def _apply_patchright_stealth(self, context, mobile_config: Dict[str, Any]):
        ...
        Patchright already has browser-level patches, so we just add
        basic JavaScript-level hiding.
        """
